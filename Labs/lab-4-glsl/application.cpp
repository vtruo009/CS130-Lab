...
GLuint loadShader(const char* vertexfilename, const char* fragmentfilename)
{
    ...
    string vertex = getTextFile(vertexfilename);
    string fragment = getTextFile(fragmentfilename);
    const char *vertex_shader_file = vertex.c_str();
    const char *fragment_shader_file = fragment.c_str();
    
    // glShaderSource
    glShaderSource(vertex_shader_id, 1, &vertex_shader_file, NULL);
    glShaderSource(fragment_shader_id, 1, &fragment_shader_file, NULL);
