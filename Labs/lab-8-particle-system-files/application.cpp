...
vec3 interpolate(vec3 v0, vec3 v1, float i) {
	//return v0 + (v1 - v0) * i;
	return v0 * (1 - i) + v1 * i;
}
...
vec3 Get_Particle_Color(float d) {
	...
	else if (d < 1.5) { //yellow to red
		return interpolate(vec3(1, 1, 0), vec3(1, 0, 0), (d - 0.1) / 1.4);
	}
	else if (d < 2) {
		return vec3(1, 0, 0);
	}
	else if (d < 3) { //red to grey
...
struct Particle {
	...
	//update v and x with an Forward Euler Step (see particle.pdf Part 1.1)
	...
	//reflect particle on ground and apply damping and restitution (see Force Sources section and Part 1.3)
	void Handle_Collision(float damping, float coeff_restitution) {
		if (position[1] < 0) {
			position[1] = 0;
			if (velocity[1] < 0) {
				velocity[1] = -1.0 * coeff_restitution * velocity[1];
...
float random(float low, float high) { //returns a random float between low and high
	return low + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX / (high - low)));
}

vector<Particle> particles; //hold a list of particles globally

// global helper functions
void Add_Particles(int n) { //generates n random particles , and appends to the particle vector
	for (int i = 0; i < n; ++i) {
		Particle p;
		p.m = 1;
		p.d = 0.0;
		...
		p.color = vec3(1, 1, 0); // need to use 1 instead of 255 cuz d would be too big for interpolation
...
void application::draw_event()
{
    ...
    if (!paused) {
        ...
        for (auto &p: particles) {
			p.Reset_forces();
			p.force = p.force + vec3(0, p.m * -9.8, 0);
			p.Euler_Step(h);
			p.Handle_Collision(1.0, 1.0);
		}
        //
        //
        //
    ...
    glBegin(GL_LINES);
        ...
        for (Particle p: particles) {
			...
			//end
			vec3 end = p.position + 0.04f * p.velocity;
